...
export function filterFeatures({
  ...
  const { column, search, geometryType, isCaseSensitive, folderId } = filter;
  ...
  for (const wrappedFeature of featureMap.values()) {
    ...
    const folderMatch = !folderId || wrappedFeature.folderId === folderId;

    if (geometryTypeMatch && folderMatch) {
...
function FeatureTableInner({ data }: { data: Data }) {
  ...
  const filter = useMemo(() => {
    return {
      ..._filter,
      // If this filter's folderId refers to a folder
      // that does not exist, nullify the folderId.
      folderId:
        _filter.folderId && folderMap.has(_filter.folderId)
          ? _filter.folderId
          : null,
    };
  }, [_filter, folderMap]);
  ...
  const handleFolderChange = useCallback(
    (e: ChangeEvent<HTMLSelectElement>) => {
      let folderId: FolderId | null = e.target.value;
              ...
              >
                <input
                  className={
                    ...
                    " mr-2 w-3 h-3 focus:ring-purple-500 focus:ring-1"
                  }
                  type="checkbox"
                  onChange={(e) => {
                    if (e.target.checked) {
